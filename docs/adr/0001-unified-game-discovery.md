...
- Keeping a discovery-specific `HistoryMode` was rejected because it hid source history and made behavior depend on how a configuration was created. Every source state is a Source Version regardless of discovery origin, while a Backup Run records the configuration-level invocation rather than defining another history mode.
...
- Every backup invocation persists an immutable Backup Run; a changed configuration state additionally produces a Configuration Checkpoint and Branch Update. Retention counts Checkpoints and protects Source Versions independently of Run presentation.
- Because this design replaces an unpublished 1.9.0 Beta, `HistoryMode`, the old discovery-origin shape, index v2, and mutable BackupRun v1 are deliberately invalidated without migration.
